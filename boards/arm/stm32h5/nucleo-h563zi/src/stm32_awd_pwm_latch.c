...
static inline void tim_set_cnt(uintptr_t base, uint32_t cnt)
{
  ...
  putreg32(cnt, base + STM32_ATIM_CNT_OFFSET);
#else
  putreg16((uint16_t)cnt, base + STM32_ATIM_CNT_OFFSET);
#endif
}

static inline void tim_clear_tif(uintptr_t base)
{
  modifyreg16(base + STM32_ATIM_SR_OFFSET, ATIM_SR_TIF, 0);
}

static inline bool tim_tif(uintptr_t base)
{
  return (getreg16(base + STM32_ATIM_SR_OFFSET) & ATIM_SR_TIF) != 0;
}
                             ...
                             uint32_t arr, uint32_t ccr)
{
  ...
  af1 = getreg32(g_base + STM32_ATIM_AF1_OFFSET);
  ...
  smcr = getreg32(g_base + STM32_ATIM_SMCR_OFFSET);
